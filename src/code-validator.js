...
export function makeSrcdoc(files) {
  ...
  const badgeBootstrap=`try{if(typeof unlockReward==='function'){const originalUnlockReward=unlockReward;unlockReward=()=>{originalUnlockReward();const target=document.getElementById('reward-content');if(!target||target.querySelector('.ec-badge'))return;const attraction=document.querySelector('h1')?.textContent||'Experience Compass';const badge=document.createElement('section');badge.className='ec-badge';badge.innerHTML='<span aria-hidden="true">✦</span><p>Achievement unlocked</p><h3></h3><small>Collector’s badge — earned by completing the experience</small>';badge.querySelector('h3').textContent=attraction;target.prepend(badge)}}}catch(error){console.error('Reward badge failed',error)}`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="${csp}"><style>${files.css}.ec-badge{margin:0 auto 20px;padding:24px;max-width:320px;text-align:center;border:2px solid #d4a72c;border-radius:50% 50% 42% 42%;background:radial-gradient(circle at 30% 20%,#fff9d8,#d4a72c);color:#17211b;box-shadow:0 12px 28px #0004}.ec-badge span{font-size:34px}.ec-badge p{margin:8px 0 2px;text-transform:uppercase;letter-spacing:.12em;font:700 11px system-ui}.ec-badge h3{margin:6px 0;font:700 25px Georgia,serif}.ec-badge small{display:block;line-height:1.35}<\/style></head><body>${files.html}<script>${files.javascript}\n${bootstrap}\n${badgeBootstrap}<\/script></body></html>`;
}
