...
export function makeSrcdoc(files) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="${csp}"><style>${files.css}.ec-badge{margin:20px auto;padding:24px;max-width:320px;text-align:center;border:2px solid #d4a72c;border-radius:50% 50% 42% 42%;background:radial-gradient(circle at 30% 20%,#fff9d8,#d4a72c);color:#17211b;box-shadow:0 12px 28px #0004}.ec-badge span{font-size:34px}.ec-badge p{margin:8px 0 2px;text-transform:uppercase;letter-spacing:.12em;font:700 11px system-ui}.ec-badge h3{margin:6px 0;font:700 25px Georgia,serif}.ec-badge small{display:block;line-height:1.35}<\/style></head><body>${files.html}<script>${files.javascript}\n${bootstrap}<\/script></body></html>`;
}
