...
// This preview is a pitch the organisation reviews before deciding to publish anything, so the
// Designer's real-world campaign idea is shown to that reviewer plainly, clearly framed as a
// recommendation rather than blended into the visitor-facing persuasive copy around it.
export function injectCampaignIdea(files={},recommendation=""){
  const html=String(files.html??"");
  const text=String(recommendation||"").trim();
  if(!text)return {...files,html:html.replaceAll("{{CAMPAIGN_IDEA}}","")};
  const callout=`<aside class="ec-campaign-idea" role="note"><p class="ec-campaign-kicker">Recommended next step for your team</p><p>${escapeHtml(text)}</p></aside>`;
  const injected=html.includes("{{CAMPAIGN_IDEA}}")?html.replaceAll("{{CAMPAIGN_IDEA}}",callout):html;
  return {...files,html:injected};
}
...
export function injectRewardBadge(files={},productType="",presentableInPerson=false){
  ...
  // The in-person line is platform-owned and lives inside the badge itself (never Maker copy)
  // so it is always both honestly hedged and correctly locked with the rest of the badge — the
  // Designer only decides, per attraction, whether it fits at all.
  const inPersonNote=presentableInPerson?`<small class="ec-badge-note">Show this during your visit — ask the team about a small surprise, subject to availability.</small>`:"";
  ...
  const badge=`<div class="ec-badge" data-ec-reward role="status" aria-label="Completion badge"><span aria-hidden="true">${badgeIcon[productType]||"✦"}</span><p>${badgeKicker[productType]||"Achievement Unlocked"}</p><h3>${attraction}</h3><small>Collector's badge — earned by completing the experience</small>${inPersonNote}</div>`;
