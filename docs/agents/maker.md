...
Give the hero container a fixed, viewport-relative height (for example 40vh–60vh) and set the img to width:100%, height:100%, object-fit:cover, so it stays a controlled cinematic crop; never leave the image at height:auto or its natural size, which renders at whatever proportions the source photo happens to have and can blow up far taller than the page.
...
Give it its own small dedicated section, positioned right after the attraction name and promise heading block and before any other Discover context (such as the place's story or history) — not folded into a card grid alongside other unrelated content.
This belongs only in Discover; never in the Reward view, which is reserved strictly for the prize the visitor actually earns by finishing the missions, not a second pitch for an unrelated future event.
...
Any copy that announces the reward as already earned — "Congratulations", "you have unlocked…", naming what the prize actually is — must live inside that same data-ec-reward element, never in the Reward view's own outer intro, because the visitor can open the Reward view at any time, including before finishing.
The platform automatically inserts its own "Complete all missions in Experience to unlock your reward" notice before the locked content, so the Reward view's own outer intro must be only a short neutral heading, for example "Your Reward" — nothing else, no body sentence beneath it at all.
Do not write any wording of your own about completing missions to unlock the reward, in any phrasing, and do not preview or name what the reward actually is there either — the platform's notice already covers the first, and the title and description belong inside data-ec-reward, not outside it.
