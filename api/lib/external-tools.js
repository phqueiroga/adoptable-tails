...
export async function searchPlaces({query,destination,open_now=false,min_rating=0,page_size=10}){
  ...
  const fields=["places.id","places.displayName","places.formattedAddress","places.location","places.primaryType","places.businessStatus","places.currentOpeningHours","places.rating","places.userRatingCount","places.priceLevel","places.accessibilityOptions","places.googleMapsUri","places.photos"];
  ...
  const results=(data.places??[]).map((place)=>{const givePhoto=!photoAssigned&&place.photos?.[0];if(givePhoto)photoAssigned=true;return {entity_id:`place:${place.id}`,place_id:place.id,label:place.displayName?.text||"Unnamed place",type:place.primaryType||null,address:place.formattedAddress||null,location:place.location||null,business_status:place.businessStatus||null,open_now:place.currentOpeningHours?.openNow??null,current_hours:place.currentOpeningHours?.weekdayDescriptions??[],rating:place.rating??null,user_rating_count:place.userRatingCount??null,price_level:place.priceLevel??null,accessibility:place.accessibilityOptions??null,source_url:place.googleMapsUri||`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.displayName?.text||query)}`,photo:givePhoto?{name:place.photos[0].name,width_px:place.photos[0].widthPx??null,height_px:place.photos[0].heightPx??null,google_maps_uri:place.photos[0].googleMapsUri??place.googleMapsUri??null,author_attributions:(place.photos[0].authorAttributions??[]).map(author=>({display_name:author.displayName??"Google Maps contributor",uri:author.uri??null,photo_uri:author.photoUri??null}))}:null}});
