...
export async function executeNextStage(run) {
  ...
  try {
    ...
    if (run.status === "building") {
      ...
      const functional = code.valid ? await testGeneratedPage(r.output.files, run.outputs.designer.selected_product, run.outputs.designer.missions, run.outputs.designer.visual_identity, run.outputs.designer.reward_strategy?.badge_presentable_in_person, run.outputs.designer.campaign_recommendation) : {valid:true,errors:[]};
