...
export async function callStructured(agent, input, tracker) {
  const isMaker=agent.name==="Maker",isDesigner=agent.name==="Designer",max_tokens=isMaker?6500:isDesigner?3000:2600;
  ...
  let message=await create(isMaker?"Build one polished visitor microsite with a hero, three navigable areas, exactly four interactions, progress and a contextual reward unlock. Keep all code below 13000 characters total and every non-code array to at most four items. No extra features.":"Return a compact complete handoff. Use no more than four short items per array and one short paragraph per scalar field.");
