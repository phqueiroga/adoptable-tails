...
export async function callResearcherWithTools(agent, briefing, definitions, executor, tracker) {
  const tools=[{type:"web_search_20250305",name:"web_search",max_uses:2},...definitions],messages=[{role:"user",content:JSON.stringify({briefing,instruction:"Call Google Places to identify the named attraction and ground it in current place data. Use web search only when historical, cultural or interpretive evidence is needed. Make no decorative calls. Return a concise research handoff that distinguishes supplied content, sourced facts and unknowns."})}],toolCalls=[];let message,hadExternalSource=false,needsFinalResponse=false;
