...
export async function callStructured(agent, input, tracker) {
  const isMaker=agent.name==="Maker",max_tokens=isMaker?6000:2400;
  ...
  let message=await create(isMaker?"Build one polished one-page visitor product with a hero, introduction, exactly three short interactions, visible progress and completion. Keep all code below 11000 characters total and every non-code array to at most three items. No extra features.":"Return a compact complete handoff. Use no more than four short items per array and one short paragraph per scalar field.");
...
export async function callResearcherWithTools(agent, briefing, definitions, executor, tracker) {
  const hasPlaces=definitions.some(tool=>tool.name==="search_places");if(!hasPlaces)throw new Error("GOOGLE_MAPS_NOT_CONFIGURED");const tools=[{type:"web_search_20250305",name:"web_search",max_uses:2},...definitions],messages=[{role:"user",content:JSON.stringify({briefing,instruction:"First identify the named attraction with Google Places. Choose precise search parameters from the briefing. After Places returns, use web search only when historical, cultural or interpretive evidence is needed. Make no decorative calls. Return a concise handoff that distinguishes supplied content, sourced facts and unknowns."})}],toolCalls=[];let message,hadExternalSource=false,needsFinalResponse=false;
  for(let turn=0;turn<3;turn++){const tool_choice=turn===0?{type:"tool",name:"search_places"}:{type:"auto"},max_tokens=turn===0?900:2400;message=await request({model,max_tokens,temperature:0.15,system:agent.system,messages,tools,tool_choice,output_config:{format:{type:"json_schema",schema:agent.schema}}},tracker,agent.name);if(message.content?.some(item=>item.type==="web_search_tool_result"||item.type==="server_tool_use"))hadExternalSource=true;const uses=message.content?.filter(item=>item.type==="tool_use")??[];if(!uses.length){needsFinalResponse=false;break}needsFinalResponse=true;messages.push({role:"assistant",content:message.content});const results=[];for(const use of uses){try{const result=await executor(use.name,use.input),id=`tool-${toolCalls.length+1}`;toolCalls.push({id,name:use.name,input:use.input,result});results.push({type:"tool_result",tool_use_id:use.id,content:JSON.stringify({source_query_id:id,...result})})}catch(error){results.push({type:"tool_result",tool_use_id:use.id,is_error:true,content:error instanceof Error?error.message:"TOOL_FAILED"})}}messages.push({role:"user",content:results})}
  if(!message)throw new Error("RESEARCHER_EMPTY_OUTPUT");if(needsFinalResponse){messages.push({role:"user",content:"Tool-use limit reached. Return the compact handoff now: at most four short items per array and only the strongest evidence."});message=await request({model,max_tokens:2400,temperature:0.1,system:agent.system,messages,output_config:{format:{type:"json_schema",schema:agent.schema}}},tracker,agent.name)}let output;try{output=parseOutput(message,agent)}catch(error){if(!(error instanceof Error)||!error.message.endsWith("_TRUNCATED_OUTPUT"))throw error;messages.push({role:"user",content:"Return the same handoff radically shortened: at most two items per array and one sentence per field. Do not call tools."});message=await request({model,max_tokens:2200,temperature:0,system:agent.system,messages,output_config:{format:{type:"json_schema",schema:agent.schema}}},tracker,agent.name);try{output=parseOutput(message,agent)}catch{throw new Error("RESEARCHER_OUTPUT_TOO_LARGE")}}if(!toolCalls.length&&!hadExternalSource)throw new Error("RESEARCHER_DID_NOT_USE_EXTERNAL_SOURCE");
