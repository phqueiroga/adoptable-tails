...
export async function callStructured(agent, input, tracker) {
  const isMaker=agent.name==="Maker",max_tokens=isMaker?5000:2400;
  ...
  let message=await create(isMaker?"Build one complete one-page prototype with exactly three short interactions. Keep all code below 7000 characters total and every non-code array to at most two items. No extra features.":"Return a compact complete handoff. Use no more than four short items per array and one short paragraph per scalar field.");
